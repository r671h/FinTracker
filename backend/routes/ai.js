...
Recent transactions (last 50): ${recentTxns.map((t) => `${new Date(t.date).toLocaleDateString()} | ${t.description} | ${t.amount.toFixed(2)} (${t.category})`).join('\n')}
  ...
  const response = await model.generateContent({
    ...
    systemInstruction: `You are a friendly, expert personal finance analyst. You have access to the user's real financial data below. Give clear, actionable, concise insights. Format numbers with 2 decimal places and currency symbol. Keep responses under 200 words unless detail is requested.\n\n${context}`
  });
